-- QPCC Tournament V2 Schema - Enhanced Features
-- This includes player voting, team balancing, supplemental draft, and division management
...
ALTER TABLE players 
ADD COLUMN IF NOT EXISTS rank_estimate TEXT CHECK (rank_estimate IN ('A', 'B', 'C')),
ADD COLUMN IF NOT EXISTS preferred_teammate_id UUID REFERENCES players(id),
...
ALTER TABLE teams 
ADD COLUMN IF NOT EXISTS team_strength_score DECIMAL(3,2) DEFAULT 0,
ADD COLUMN IF NOT EXISTS division TEXT DEFAULT 'main',
...
CREATE TABLE trades (
  ...
  balance_impact DECIMAL(3,2), -- how this affects team balance
  notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
...
-- 6. Team Requests Table (for drops, division changes)
...
-- INDEXES
CREATE INDEX idx_player_votes_voter ON player_votes(voter_id);
CREATE INDEX idx_player_votes_subject ON player_votes(subject_id);
CREATE INDEX idx_supplemental_players_draft_order ON supplemental_players(draft_order) WHERE drafted_to_team_id IS NULL;
...
SELECT 
  ...
  ROW_NUMBER() OVER (
    ORDER BY 
      player_count ASC,  -- Teams with fewer players pick first
      strength_score DESC  -- Among equal sizes, weaker teams pick first
  ) as draft_position
FROM team_stats
...
BEGIN
  ...
  WHERE p.id IN (NEW.subject_id, OLD.subject_id);
...
-- Everyone can read votes but only admins can write
